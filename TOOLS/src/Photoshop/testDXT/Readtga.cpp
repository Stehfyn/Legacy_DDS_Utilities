...
NV_ERROR_CODE TGA::ReadTGA(const char * filename, RGBAImage & image, bool bFlipVertical)
{
    FILE * fp = fopen( filename, "rb" );

    if( NULL != fp )
        return ReadTGA(  fp, image, bFlipVertical);
    else
    {
...
NV_ERROR_CODE TGA::ReadTGA(FILE * fp, RGBAImage & image, bool bFlipVertical)
{
    int rle;
    int a, r, g, b;
    int bFlipLeftRight;
    ...
    bFlipLeftRight = m_desc & LR;  // swap left/right, not supported
    bFlipVertical = m_desc & BT;    // file requested flip

    //if (format & TGA_FLIP_VERTICAL)
    //    bFlipVertical = TRUE;
    ...
    if (m_type == 2)
    {
        ...
        case 16:
            for (int y = 0; y < m_height; y++)
            {
                ...
                {
                    rgba_t & pix = image(m_height - y - 1, x);
